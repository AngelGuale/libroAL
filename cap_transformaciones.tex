...
\begin{dfn}
Sean $V$ y $W$ dos espacios vectoriales sobre un campo $\mathbb{K}$. Una función $L: V \to W$ se dice que es lineal si para todo $u$, $v$ en $V$ y todo $\lambda \in \mathbb{K}$, se cumple que
\begin{enumerate}
\item $L (u+v) = L(u) + L(v)$
...
\begin{ejemplo}
Sea $r_0 \in \mathbb{K}$ y $V$ un espacio vectorial sobre un campo $\mathbb{K}$. La función 
\begin{align*}
L: V &\to V\\
L(v) &= r_0 v
\end{align*}
...
\begin{theorem}
\label{th:ag}
Sean $V$ y $W$ dos espacios vectoriales y $T: V \to W$ una transformación lineal. Si $B = \conjvect{v}{n}$ es una base para $V$ entonces $L$ está unívocamente determinada por los vectores \llav{T(v_1), T(v_2), \ldots , T(v_n)}.
\end{theorem}
...
\begin{proof}
En efecto sea $L: V \to W$ la función definida por 
$$L(v) = L\left(\sum_{i = 1}^n \alpha_i v_i\right) = \sum_{i = 1}^n \alpha_i L(v_i) = \sum_{i = 1}^n \alpha_i w_i$$.
\\
